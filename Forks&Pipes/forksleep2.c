...
int main(void) {
  ...
  if (fork_pid == 0) {
    elp_time = get_elapsed_time();
    printf("1st Hello from Child1  %14.6lf\n", elp_time);
    sleep(SLEEP_TIME);
    elp_time = get_elapsed_time();
    printf("2nd Hello from Child1  %14.6lf\n", elp_time);
  } else {
    ...
    {
      elp_time = get_elapsed_time();
      printf("1st Hello from Child2  %14.6lf\n", elp_time);
      sleep(SLEEP_TIME);
      elp_time = get_elapsed_time();
      printf("2nd Hello from Child2  %14.6lf\n", elp_time);
    }
    else
    {
      int status; int wpid;
      ...
      wpid = wait(&status);
      elp_time = get_elapsed_time();
      printf("3nd Hello from Parent wpid = %d exited = %d %14.6lf\n",
        wpid, WIFEXITED(status), elp_time);
    }
  }
